...
function solution(N) {
  ...
  let n = N; // personally prefer lowercase 'n'
  let tempcount = 0;
  let countingStarts = false;
  let maxcount = 0;
  ...
  while(true){
    ...
    // party begins
    if(!countingStarts && leftover === 1) {
      countingStarts = true;

    }else if (countingStarts && leftover === 0) {
      // counting begins
      tempcount++;

    }else if(countingStarts && leftover === 1 && tempcount > 0){
      //party is over
